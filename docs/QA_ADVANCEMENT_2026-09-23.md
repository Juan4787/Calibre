...
La campaña final corresponde al commit de código `4f368d4586438333a1c1658ec1cb09fe34a20dec` y a la huella de código, pruebas y reglas de checkout `bf5b45d760d225038f2bc0e1e01dca4075e64c089c6590b61480c640f5482371`. Los nueve recibos aceptados, sus logs, reportes resumidos y los artefactos de ambas ejecuciones Linux/Windows se conservan en [`qa-evidence/2026-09-23/`](qa-evidence/2026-09-23/README.md). El ledger coteja los recibos con esa huella y con el SHA-256 de cada log. Sus rutas originales eran locales e ignoradas por Git; las copias publicables sólo cambian la ubicación del log y registran el SHA-256 del recibo original. Esto permite inspeccionar la evidencia en GitHub, aunque no sustituye repetir los comandos en otro entorno.

| Comprobación | Resultado de esta corrida | Alcance que acredita |
|---|---|---|
| `verify_delivery.py` | 470 archivos versionados comparados; dos builds limpios con contenido idéntico por miembro para wheel (22 miembros) y sdist (239); instalación separada de ambos paquetes y smoke de UI, demo, SQLite, replay, ZIP y paridad económica LF/CRLF. Ambiente aislado: 38 dependencias del lock cotejadas. | Linux de esta máquina; los archivos `.whl` y `.tar.gz` **no** fueron idénticos byte a byte. El reporte conserva ambos hashes y el digest de contenido; no se atribuye la diferencia de bytes a una causa sin probarla. |
| `scripts/verify.sh` | Lint, formato, mypy, matriz, sintaxis JS, 230 pruebas, compilación y build pasaron secuencialmente. | Dos advertencias `StarletteDeprecationWarning` de TestClient; ningún fallo de pruebas. |
| `qa.py mutate --execute` | 10/10 mutantes abatidos por la señal y el selector previstos, con baseline limpio. | Esas diez defensas; no un porcentaje global de mutación. |
| `browser_e2e.py` | Chromium 153 leyó 46 filas reales del DOM y cotejó JSON/XLSX/HTML/API. Otro origen no pudo leer la sesión ni crear una corrida; escucha IPv4 observada sólo en `127.0.0.1`. | Linux/Chromium y ese flujo local; no demuestra firewall ni aislamiento de usuarios del sistema. |
| Pipeline sintético de 1.000 cargos | 13/13 etapas, 1.000 hallazgos, oráculo exacto ARS/USD; 19,04 s y pico RSS 184,95 MB en una repetición. | Sólo ese tamaño, equipo y configuración; sin extrapolación a 50.000/100.000. |
| Control documental externo ficticio | Corrida exportada, `verify-bundle --replay` conforme y cotejo CLI conforme para 32 operaciones, 46 cargos y 43.719,4 ARS. | Demuestra el contrato del verificador con un control sintético; la independencia/verdad del control real permanece por probar. |
| [Workflow Linux/Windows](https://github.com/Juan4787/Calibre/actions/runs/35823451264) | 17 fixtures con bytes idénticos al commit en ambos sistemas; 24/24 casos en Linux, 24/24 en Windows, nueve pruebas adicionales desde el wheel instalado en Windows y tres datasets con huellas semánticas completas iguales (cero divergencias). | Runners hospedados y esos fixtures; no Excel de escritorio, instalación en la PC objetivo ni contrato real. |

La primera pasada de `verify.sh` detectó cuatro anotaciones de tipos faltantes en el nuevo control externo; se corrigieron antes de la pasada final. La primera ejecución del verificador de entrega confundía un CSV CRLF del checkout con el blob LF del commit; se cambió a comparar el contenido comprometido en Git. La segunda reveló un error de tipo al comprobar la lista de miembros del wheel; también se corrigió. Los tres recibos fallidos están archivados como **incidentes**, no como evidencia verde. Estas correcciones están incluidas en la huella final. Los comandos completos y hashes aceptados constan en el ledger; una corrida anterior a esa huella queda histórica, no vigente.

El [primer workflow Linux/Windows](https://github.com/Juan4787/Calibre/actions/runs/35822778429) encontró una divergencia real de bytes de checkout: `fixtures/authorization.txt` era LF en Linux y CRLF en Windows. Los 24 casos de cada sistema estaban verdes, pero el hash del original cambió el snapshot y el ID de la corrida. Convertir sólo ese archivo a CRLF en una copia aislada reprodujo **exactamente** ambos hashes de Windows; los otros campos de la huella, incluidos los 46 hallazgos, eran iguales. Se fijó `eol=lf` para ese original, se añadió una comprobación previa de los 17 fixtures contra los blobs Git en ambos runners y `.gitattributes`/`.gitignore` entraron en la huella de evidencia. El workflow posterior completó la conciliación. Ambos conjuntos de artefactos y el reproducer resumido están archivados; el primer resultado permanece rojo, no se reinterpreta como éxito.
...
| QA-41 | Los nueve casos nuevos pasaron en el runner Windows; falta ensayar el sistema de archivos Windows objetivo, cambios concurrentes de rutas y corte durante backup/exportación. |
...
| QA-47 | Wheel clean-room y tres huellas pasaron en Windows; faltan instalación de sdist en Windows, Excel de escritorio y entorno Windows objetivo. |
...
| QA-55 | Completar sdist/instalador y Excel en Windows objetivo, hashes de dependencias, firma/distribución y actualización verificable. |
